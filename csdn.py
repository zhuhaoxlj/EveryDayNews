...
url = 'https://blog.csdn.net/phoenix/web/blog/hot-rank?page=0&pageSize=100&type='
...
# 获取自己关注的文章
def getMyFocusArticle(my_favorite_type, article_list,is_show):
    my_favorite_article_list = []
    count = 0
    for i in article_list:
        if any(type in i.articleTitle.lower() for type in my_favorite_type):
            my_favorite_article_list.append(i)
            count += 1
            if is_show:
                print("***************************")
    ...
    return my_favorite_article_list



if __name__=="__main__":
    getMyFocusArticle(my_favorite_type.split('\n')[1:-1],get_rank_article_list())
